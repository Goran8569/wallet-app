...
app.get("/transactions", authenticateToken, (req, res) => {
  const { page = 1, per_page = 15, wallet_id, type, status, date_from, date_to, search } = req.query;
  ...
  if (search) {
    filteredTransactions = filteredTransactions.filter((t) => t.reason.toLowerCase().includes(search.toLowerCase()));
  }

  if (date_from) {
    filteredTransactions = filteredTransactions.filter((t) => new Date(t.created_at) >= new Date(date_from));
  }

  if (date_to) {
    filteredTransactions = filteredTransactions.filter((t) => new Date(t.created_at) <= new Date(date_to));
  }
